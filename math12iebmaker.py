...
class Math12IEBMaker(AbstractRectangleMaker):
    ...
    #use get_stems to get the list, then pass it to the rectangles function to get the rects. 
        ...
        qstem_list=utils.map_stems(self.q_list,qstem_list)
    ...
    def export_rectangles(self):
        ...
        self.q_list[:]=utils.flatten_list(self.q_list)
        rects={
            "q_rects":q_rects,
            ...
            "q_titles":q_titles,
            "q_texts":self.q_list,
            "rect_check": (len(q_rects)==len(a_rects)==len(q_titles)==len(self.q_list)),
